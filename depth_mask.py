...
class ImageMask():
    def __init__(self):
        ...
        rospy.Subscriber("depth_mask/IO",Bool,self.set_depth_mask)
        rospy.loginfo("set depth mask")
        self.setDepthMask: Bool = True
    ...
    #深度情報をつけるかつけないか
    def set_depth_mask(self,data):
        self.setDepthMask = data.data
        

    #深度情報を処理
    def depth_listener(self,data):
        if(self.setDepthMask):
            try:
            ...
            print("depth_mask is not set")
            self.imshow(self.img)
            img_pub = self.bridge.cv2_to_imgmsg(self.img,encoding="bgr8")
